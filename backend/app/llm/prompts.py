...
- 每个 issue 必须包含：
  ...
  - original：译文中需要修改的原文片段（必须使用 {target_language}，不得包含中文字符）
  - suggestion：修改建议（必须使用 {target_language}，不得包含任何中文字符、拼音、罗马化中文或中英混用）
  - explanation：为什么需要修改（中文，50字以内）
  - source_reference：对应的中文原文片段（如有）

重要：suggestion、original 和 span.text 必须使用 {target_language} 撰写，不得出现中文字符、拼音或中英混用。explanation 和 summary 仍使用中文。
...
- 每个 issue 必须包含：
  ...
  - severity："low"、"medium"、"high"
  - span：{{"start": 字符偏移, "end": 字符偏移, "text": "译文中的对应文本"}}
  - original：译文中需要修改的片段（必须使用 {target_language}，不得包含中文字符）
  - suggestion：修改建议（必须使用 {target_language}，不得包含任何中文字符、拼音、罗马化中文或中英混用）
  - explanation：为什么需要修改（中文，50字以内）
