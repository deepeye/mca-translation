...
AcceptanceAggregator: 句级 → 全文
  ...
  全文置信度 = min(句置信度)  // 任一句低置信则全文标低
        │
        ▼
写入 TranslationResult（acceptance_score 已有列 / audience_baseline 已有列 /
                     acceptance_confidence / dimensions JSON / sentence_scores JSON 新列）
...
字段加在 `TranslationResult` 上（每语言一行译文一个评分），不另建实体。`TranslationResult` **已存在** `acceptance_score`（`Integer, default=-1`，PRD 第 596 行预留）和 `audience_baseline`（`String(32), nullable`，PRD 第 597 行预留）两列，且已有 `risk_annotations` JSONB（接受度评分消费它）。本功能只需新增 3 列。

> 注：PRD 第 596–597 行的字段在现有模型中落在 `TranslationResult`（每语言粒度）而非 `TranslationJob`（任务粒度）——更合理，因为接受度是按目标语言译文算的。本设计沿用此结构。

### `TranslationResult` 新增字段
