...
| backend | `python -c "import urllib.request; urllib.request.urlopen('http://localhost:8000/health')"` | interval: 10s, timeout: 5s, retries: 3 |
| frontend | `wget -qO- http://localhost:3000/` | interval: 10s, timeout: 5s, retries: 3 |
| nginx | `wget -qO- http://localhost/health` | interval: 10s, timeout: 5s, retries: 3 |
...
- `docker-compose.yml` 中为 frontend 服务设置 `HOSTNAME=0.0.0.0`，确保 Next.js standalone 在容器内监听所有接口。
- nginx 配置 `client_max_body_size 10m`，匹配后端文件上传大小限制。
- 根目录 `.env.example` 汇总了部署所需的全部变量，包括 `BAILIAN_API_KEY`、`SECRET_KEY`、`DB_PASSWORD`、`NEXT_PUBLIC_API_URL`、`NEXT_PUBLIC_WS_URL`、`FRONTEND_URL`、`RUN_MIGRATIONS`。
